...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.output and len(input_files) > 1:
        print("Error: Cannot use -o/--output with multiple input files. Use -d/--output-dir instead.",
              file=sys.stderr)
        return 1
    ...
    if len(input_files) > 1 and not args.quiet:
        print(f"\nProcessed {len(input_files)} files: {success_count} success, {error_count} errors")
