...
class HTMLConverter:
    ...
    def convert_fragment(self, tokens: List[Token]) -> str:
        """
        Convert tokens to HTML without document wrapper.
        
        Useful for embedding converted Markdown in existing HTML pages.
    ...
    def _generate_toc(self) -> str:
        ...
        Creates a nested list structure reflecting the header hierarchy.
        ...
        for header in self._headers:
            ...
            # Strip HTML tags from content for TOC display
            clean_content = self._strip_html_tags(header.content)
            toc_parts.append(
                f'<li><a href="#{header.id}">{clean_content}</a></li>'
            )
