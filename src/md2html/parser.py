...
class MarkdownParser:
    ...
    # Maximum characters an emphasis span (bold/italic) may cover. Bounding
    # this lazy quantifier prevents catastrophic backtracking (ReDoS) on
    # adversarial input containing many unmatched `*`/`_` characters, while
    # remaining far larger than any realistic emphasized phrase or sentence.
    MAX_EMPHASIS_SPAN = 200

    # Regex patterns for inline elements
    INLINE_PATTERNS = {
        # Bold + Italic: ***text*** or ___text___
        "bold_italic": re.compile(
            r"(\*{3}|_{3})(?!\s)(.{1,%d}?)(?<!\s)\1" % MAX_EMPHASIS_SPAN,
            re.DOTALL,
        ),
        # Bold: **text** or __text__
        "bold": re.compile(
            r"(\*{2}|_{2})(?!\s)(.{1,%d}?)(?<!\s)\1" % MAX_EMPHASIS_SPAN,
            re.DOTALL,
        ),
        # Italic: *text* or _text_ (not preceded/followed by same char)
        "italic": re.compile(
            r"(?<![*_])(\*|_)(?!\s)(.{1,%d}?)(?<!\s)\1(?![*_])" % MAX_EMPHASIS_SPAN,
            re.DOTALL,
        ),
    ...
    # Placeholder used to protect inline code spans from later regex passes.
    # Uses NUL bytes, which are stripped from all incoming text in
    # _escape_html, so this token can never collide with user content.
    _CODE_SPAN_MARKER = "\x00CODESPAN{}\x00"

    def __init__(self):
        """Initialize the parser with empty state."""
        self._tokens: List[Token] = []
        self._headers: List[Token] = []  # Track headers for TOC generation
        self._slug_counts = {}  # Track slug usage to keep header IDs unique
    ...
    def _parse_header(self, match: re.Match) -> None:
        ...
        # Generate a slug for the header ID, deduplicating repeats so
        # duplicate headings (e.g. "## Overview" appearing twice) don't
        # collide on the same anchor.
        header_id = self._generate_unique_slug(content)

        # Parse inline elements in header content
        parsed_content = self._parse_inline(content)

        token = Token(
            type=TokenType.HEADER, content=parsed_content, level=level, id=header_id
        )
    ...
    def _parse_paragraph(self, lines: List[str], start: int) -> int:
        ...
        while i < len(lines):
            ...
            if (
                ...
                or line.startswith("    ")
                or line.startswith("\t")
                or self.PATTERNS["hr"].match(line)
                or self.PATTERNS["ul_item"].match(line)
                or self.PATTERNS["ol_item"].match(line)
            ):
                break
        ...
        if paragraph_lines:
            # Join with real newlines (not spaces) so trailing-double-space
            # hard line breaks survive into inline parsing. Soft-wrapped
            # lines without a hard break are folded back into a single
            # line inside _parse_inline.
            content = "\n".join(paragraph_lines)
            parsed_content = self._parse_inline(content)
            self._tokens.append(Token(type=TokenType.PARAGRAPH, content=parsed_content))
    ...
    def _parse_indented_code_block(self, lines: List[str], start: int) -> int:
        ...
        self._tokens.append(
            Token(type=TokenType.CODE_BLOCK, content="\n".join(code_lines))
        )
    ...
    def _parse_blockquote(self, lines: List[str], start: int) -> int:
        ...
        self._tokens.append(
            Token(type=TokenType.BLOCKQUOTE, content=parsed_content, level=1)
        )
    ...
    def _parse_unordered_list(self, lines: List[str], start: int) -> int:
        ...
        while i < len(lines):
            ...
            if match:
                ...
                parsed_content = self._parse_inline(content)
                items.append(
                    Token(type=TokenType.LIST_ITEM, content=parsed_content, level=level)
                )
                i += 1
            elif self.PATTERNS["empty_line"].match(line):
        ...
        self._tokens.append(Token(type=TokenType.UNORDERED_LIST, children=items))
    ...
    def _parse_ordered_list(self, lines: List[str], start: int) -> int:
        ...
        self._tokens.append(Token(type=TokenType.ORDERED_LIST, children=items))
    ...
    def _parse_inline(self, text: str) -> str:
        ...
        # 2. Stash inline code spans before any other inline processing.
        #    Code span content must not be re-interpreted by later bold,
        #    italic, link, or image regexes (e.g. `**not bold**` inside
        #    backticks must render literally).
        code_spans = []

        def _stash_code(match):
            code_spans.append(match.group(1))
            return self._CODE_SPAN_MARKER.format(len(code_spans) - 1)

        text = self.INLINE_PATTERNS["code_inline"].sub(_stash_code, text)

        # 3. Process images before links (images start with !)
        text = self.INLINE_PATTERNS["image"].sub(self._replace_image, text)

        # 4. Process links
        text = self.INLINE_PATTERNS["link"].sub(self._replace_link, text)
        ...
        text = self.INLINE_PATTERNS["bold"].sub(r"<strong>\2</strong>", text)

        # 7. Process italic (* or _)
        text = self.INLINE_PATTERNS["italic"].sub(r"<em>\2</em>", text)

        # 8. Convert hard line breaks (two or more trailing spaces before a
        #    newline) to <br>. Any remaining newline is a soft wrap from the
        #    original source line and folds back into a single line.
        text = self.INLINE_PATTERNS["line_break"].sub("<br>", text)
        text = text.replace("\n", " ")

        # 9. Restore stashed code spans last, after emphasis/line-break
        #    processing has already skipped over their placeholder tokens.
        for index, content in enumerate(code_spans):
            text = text.replace(
                self._CODE_SPAN_MARKER.format(index), f"<code>{content}</code>"
            )
    ...
    def _escape_html(self, text: str) -> str:
        ...
        text = text.replace("\x00", "")  # never let raw input collide with our markers
        text = text.replace("&", "&amp;")
        text = text.replace("<", "&lt;")
        text = text.replace(">", "&gt;")
        return text
    ...
    def _generate_unique_slug(self, text: str) -> str:
        """
        Generate a slug for a header, disambiguating duplicates.

        Repeated headings (identical text, or text that slugifies to the
        same value) would otherwise produce duplicate `id` attributes,
        breaking in-page anchors and the table of contents. The first
        occurrence keeps the plain slug; later occurrences get a
        `-2`, `-3`, ... suffix.
        ...
        Returns:
            A unique, lowercase, hyphenated slug suitable for an HTML id.
        """
        base = self._generate_slug(text)
        seen = self._slug_counts.get(base, 0)
        self._slug_counts[base] = seen + 1
